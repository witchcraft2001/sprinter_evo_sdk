...
tfmpresent
	...
	ld a,SND_PAGE
	call setSlot1
	;почему-то не выставляется количество эффектов
	;ld a,(SFX_COUNT)
	;or a
	;jr z,.noSfx
	ld hl,SFX_DATA
...
;a'=page for slot 2
	...
	push hl
       if 0;ifdef EVO
	ld bc,#ff77 ;shadow ports off, but still on by port #bf
       else
	ld bc,#bd77 ;shadow ports on
       endif
	ld a,%10101011 ;6912
	out (c),a ;set 6912
	ld a,#7f-#05
...
.sfx
	...
	poll_mouse
	;call poll_joystick ;нельзя в прерывании, т.к. нужно знать содержимое MEM_SLOT0
