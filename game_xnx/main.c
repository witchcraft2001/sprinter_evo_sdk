...
#ifdef __SPRINTER__
// Sprinter: Esc приходит в матрицу #FE как CS+Space (см. manual). Esc в игре
// возвращает в меню (этот флаг), Esc в меню выходит в DSS (quit_to_dss).
static u8 gameQuit;
#endif
...
u8 input_fire_pressed(void)
{
	keyboard(keys);
#ifdef __SPRINTER__
	// Esc = CS+Space; огонь/старт -- только чистый Space (без CS), иначе мисклик.
	if(keys[KEY_CAPS]&KEY_DOWN) return (joystick()&JOY_FIRE)!=0;
#endif
...
#ifdef __SPRINTER__
		// Esc (CS+Space) -- прервать уровень и вернуться в меню.
		if((keys[KEY_SPACE]&KEY_PRESS)&&(keys[KEY_CAPS]&KEY_DOWN))
		{
			sprites_stop();
			music_stop();
			fade_to_black();
...
#ifdef __SPRINTER__
		// Esc (CS+Space) в меню -- выход в DSS. input_fire_pressed уже не
		// считает это огнём (CS зажат), поэтому мисклика "старт" не будет.
		else if((keys[KEY_SPACE]&KEY_PRESS)&&(keys[KEY_CAPS]&KEY_DOWN)) quit_to_dss();
#endif
...
void main(void)
{
	...
	while(1)
	{
		...
		while(1)
		{
			...
			{
#ifdef __SPRINTER__
				// Esc прервал игру -- вернуться в меню без экрана gameover.
				if(gameQuit){ gameQuit=FALSE; break; }
#endif
				gameover_screen();
				break;
			}
